<?php  function lcr595cec7892c58ifvar($cx, $v, $zero) {
  return ($v !== null) && ($v !== false) && ($zero || ($v !== 0) && ($v !== 0.0)) && ($v !== '') && (is_array($v) ? (count($v) > 0) : true);
 }

 function lcr595cec7892c58sec($cx, $v, $bp, $in, $each, $cb, $else = null) {
  $push = ($in !== $v) || $each;
  ...
  if (($loop && $isAry) || $isTrav) {
   ...
   foreach ($v as $index => $raw) {
    ...
    if (isset($bp[0])) {
     $raw = lcr595cec7892c58m($cx, $raw, array($bp[0] => $raw));
    }
    if (isset($bp[1])) {
     $raw = lcr595cec7892c58m($cx, $raw, array($bp[1] => $cx['sp_vars']['index']));
    }
    $ret[] = $cb($cx, $raw);
   }
   if ($cx['flags']['spvar']) {
 ...
 function lcr595cec7892c58m($cx, $a, $b) {
  if (is_array($b)) {
   if ($a === null) {
    return $b;
   } else if (is_array($a)) {
...
return function ($in = null, $options = null) {
    ...
    $inary=is_array($in);
    ob_start();echo '<section class="enp-tree">
