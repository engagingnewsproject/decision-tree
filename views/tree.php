<?php use \LightnCandy\SafeString as SafeString;use \LightnCandy\Runtime as LR;return function ($in = null, $options = null) {
    $helpers = array(            'environment' => function($options) {
        return 'no-js';
            ...
            'groupStart' => function($questionID, $groupID, $groups, $options) {
        ...
        return '';
            ...
            'groupEnd' => function($questionID, $groupID, $groups, $options) {
        foreach($groups as $group) {
            if($group['groupID'] === $groupID) {
                // check if it's the first in the question order
    ...
    $inary=is_array($in);
    return '<section id="cme-tree--'.LR::raw($cx, (($inary && isset($in['treeID'])) ? $in['treeID'] : null)).'" class="cme-tree cme-tree--'.LR::raw($cx, LR::hbch($cx, 'environment', array(array(),array()), 'raw', $in)).'">
    ...
    <div class="cme-tree__intro-wrapper">
        <div id="cme-tree__intro--'.LR::raw($cx, (($inary && isset($in['treeID'])) ? $in['treeID'] : null)).'" class="cme-tree__intro">
...
' : '').'        </div>
    ...
    <div id="cme-tree__content-wrapper--'.LR::raw($cx, (($inary && isset($in['treeID'])) ? $in['treeID'] : null)).'" class="cme-tree__content-wrapper">
        <div id="cme-tree__content-window--'.LR::raw($cx, (($inary && isset($in['treeID'])) ? $in['treeID'] : null)).'" class="cme-tree__content-window">
            <div id="cme-tree__content-panel--'.LR::raw($cx, (($inary && isset($in['treeID'])) ? $in['treeID'] : null)).'" class="cme-tree__content-panel">
'.((LR::ifvar($cx, (($inary && isset($in['questions'])) ? $in['questions'] : null), false)) ? '                <div class="cme-tree__questions">
'.LR::sec($cx, (($inary && isset($in['questions'])) ? $in['questions'] : null), null, $in, true, function($cx, $in) {$inary=is_array($in);return ''.((LR::ifvar($cx, (($inary && isset($in['groupID'])) ? $in['groupID'] : null), false)) ? ''.LR::hbbch($cx, 'groupStart', array(array((($inary && isset($in['questionID'])) ? $in['questionID'] : null),(($inary && isset($in['groupID'])) ? $in['groupID'] : null),((isset($cx['scopes'][count($cx['scopes'])-1]) && is_array($cx['scopes'][count($cx['scopes'])-1]) && isset($cx['scopes'][count($cx['scopes'])-1]['groups'])) ? $cx['scopes'][count($cx['scopes'])-1]['groups'] : null)),array()), $in, false, function($cx, $in) {$inary=is_array($in);return '                                <div id="cme-tree__el--'.LR::raw($cx, (($inary && isset($in['groupID'])) ? $in['groupID'] : null)).'" class="cme-tree__group">
                                    <h3 class="cme-tree__title cme-tree__title--group">'.LR::encq($cx, (($inary && isset($in['groupTitle'])) ? $in['groupTitle'] : null)).'</h3>
';}).'' : '').'                        <section id="cme-tree__el--'.LR::raw($cx, (($inary && isset($in['questionID'])) ? $in['questionID'] : null)).'" class="cme-tree__question" tabindex="-1">
...
'.LR::sec($cx, (($inary && isset($in['options'])) ? $in['options'] : null), null, $in, true, function($cx, $in) {$inary=is_array($in);return '                                        <li class="cme-tree__option"><a  class="cme-tree__option-link" id="cme-tree__el--'.LR::raw($cx, (($inary && isset($in['optionID'])) ? $in['optionID'] : null)).'" href="#cme-tree__el--'.LR::raw($cx, (($inary && isset($in['destinationID'])) ? $in['destinationID'] : null)).'"
                                        aria-describedby="cme-tree__el--'.LR::raw($cx, (($inary && isset($in['questionID'])) ? $in['questionID'] : null)).'">
...
'.LR::sec($cx, (($inary && isset($in['ends'])) ? $in['ends'] : null), null, $in, true, function($cx, $in) {$inary=is_array($in);return '
                        <section id="cme-tree__el--'.LR::raw($cx, (($inary && isset($in['endID'])) ? $in['endID'] : null)).'" class="cme-tree__end" tabindex="-1">
...
' : '').'
                            <ul class="cme-tree__end-options">
                                <li class="cme-tree__end-option">
                                    <a id="cme-tree__restart--'.LR::encq($cx, (($inary && isset($in['endID'])) ? $in['endID'] : null)).'" class="cme-tree__btn cme-tree__btn--retry" href="#cme-tree__el--'.LR::raw($cx, ((isset($cx['scopes'][count($cx['scopes'])-1]) && is_array($cx['scopes'][count($cx['scopes'])-1]['questions']['0']) && isset($cx['scopes'][count($cx['scopes'])-1]['questions']['0']['questionID'])) ? $cx['scopes'][count($cx['scopes'])-1]['questions']['0']['questionID'] : null)).'">Start Again</a>
                                </li>
                                <li class="cme-tree__end-option">
                                    <a id="cme-tree__overview--'.LR::encq($cx, (($inary && isset($in['endID'])) ? $in['endID'] : null)).'" class="cme-tree__btn cme-tree__btn--overview" href="#cme-tree--'.LR::raw($cx, ((isset($cx['scopes'][count($cx['scopes'])-1]) && is_array($cx['scopes'][count($cx['scopes'])-1]) && isset($cx['scopes'][count($cx['scopes'])-1]['treeID'])) ? $cx['scopes'][count($cx['scopes'])-1]['treeID'] : null)).'">Go to Overview</a>
                                </li>
                            </ul>
                        </section>
';}).'                </div>
