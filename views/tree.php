...
'.LR::sec($cx, (($inary && isset($in['options'])) ? $in['options'] : null), null, $in, true, function($cx, $in) {$inary=is_array($in);return '                            <li id="enp-tree__option--'.LR::raw($cx, (($inary && isset($in['option_id'])) ? $in['option_id'] : null)).'" class="enp-tree__option"><a class="enp-tree__option-link"  href="#enp-tree__destination--'.LR::raw($cx, (($inary && isset($in['destination_id'])) ? $in['destination_id'] : null)).'">'.LR::encq($cx, (($inary && isset($in['title'])) ? $in['title'] : null)).'</a></li>
';}).'                    </ul>
' : '').'            </section>
...
'.((LR::ifvar($cx, (($inary && isset($in['description'])) ? $in['description'] : null), false)) ? '                    <div class="enp-tree__description enp-tree__description--end">'.LR::encq($cx, (($inary && isset($in['description'])) ? $in['description'] : null)).'</div>
' : '').'                <a class="enp-tree__btn enp-tree__btn--retry" href="#enp-tree__destination--'.LR::encq($cx, ((isset($cx['scopes'][count($cx['scopes'])-1]) && is_array($cx['scopes'][count($cx['scopes'])-1]['questions']['0']) && isset($cx['scopes'][count($cx['scopes'])-1]['questions']['0']['question_id'])) ? $cx['scopes'][count($cx['scopes'])-1]['questions']['0']['question_id'] : null)).'">Start Again</a>
            </section>
';}).'' : '').'
