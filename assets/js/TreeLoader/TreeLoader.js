...
(function () {
  ...
  function TreeLoader() {
    ...
    * Private functions
    */
    var _setRootURL = function(url) {
        // regex it to see if it's one of our DEV urls
        let regex = /https?:\/\/(?:(?:localhost:3000|decision-tree.test)|(?:enptree|cmetree)\.(?:staging\.)?wpengine\.com)\b/
    ...
    var _setTreeSlug = function(slug) {
    ...
    var _setTreeStyle = function(style) {
        // Get Style
        if(style === null || style === '') {
            _treeStyle = 'base'
        } else {
    ...
    var _setCSSPriority = function(style) {
        // Test string for a match where we'll need to set !important
        let re = /.+(important|clean-slate)$/
        if(re.test(style)) {
    ...
    var _setLoaderScript = function(script) {
        // Set the script
    ...
    this.getTreeSlug = function() { return _treeSlug}
    this.getTreeStyle = function() { return _treeStyle}
    this.getCSSPriority = function() { return _cssPriority}
    this.getLoaderScript = function() { return _loaderScript}
    this.getTreeContainer = function() { return _treeContainer}
